...
The code is specifically designed to handle non-Markovian decays, to model super-spreading in epidemics [1]. An _event_ is a dictionary element of whose key is an epidemic time and whose value is a dictionary of decays that occur at the epidemic time. A heap sorts the dictionary keys, the times, so the time of the next decay can be determined, and if desired popped from the heap-dictionary. The computation ensures that the dictionary of events holds all future transitions, holds future non-Markovian decays. 
